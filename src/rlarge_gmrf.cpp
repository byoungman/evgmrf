...
double rlargegmrfld0_omp(const arma::mat& pars, const arma::field<arma::mat>& yc, const arma::field<arma::mat>& wc, int threads = 0)
{
...
#pragma omp parallel reduction(+:nllh)
{
  ...
  for (int j=0; j < n; j++) {
    ...
    int m = yv.n_rows; // number of obs
    int r = yv.n_cols; // number of order statistics
    ...
    for (int k=0; k < m; k++) {
      ...
      for (int l=0; l < r; l++) {
        ...
        if (std::isfinite(y[l])) {
          ...
          if (ee1[l] <= -1.0) {
            ...
            local_nllh += w[l] * (lpsi + (ee2 + 1.0) * std::log1p(ee1[l]));
      ...
      local_nllh += w[r - 1] * std::pow(1 + ee1[r - 1], -ee2);
...
arma::mat rlargegmrfld12_omp(const arma::mat& pars, const arma::field<arma::mat>& yc, const arma::field<arma::mat>& wc, int threads = 0)
{
